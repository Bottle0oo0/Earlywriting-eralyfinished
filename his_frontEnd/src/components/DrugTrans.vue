<template>
  <div>
    <div class="head">
      ...
      <el-button type="primary" icon="el-icon-search" plain circle style="margin-right: 80px" @click="searchDrugs(registerFormId)"></el-button>
      <span>身份证号：</span>
    ...
    <div class="foot">
      <el-button type="primary" plain @click="drugTrans()">确认出药</el-button>
      <el-button type="warning" plain @click="cancel()">取消出药</el-button>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    drugTrans() {
      if (this.patient.name == null) {
        ...
        axios.post("/drug_trans", {affairs: this.selected})
        this.cancel()
        this.$message("出药成功")
      }
    },
